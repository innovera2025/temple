...
**users**
- `tenant_id`, `name`, `email` (unique ต่อ tenant), `password_hash`, `role` [`admin|finance|staff`], `status` [`active|disabled`], `last_login_at?`
- Canonical access groups are mapped from real roles: `platform_owner` = platform `super_admin|support`, `temple_owner` = tenant `admin`, `temple_user` = tenant `finance|staff`. `auditor`/`viewer` are not product runtime roles unless a future schema/API/seed migration adds them deliberately.
...
| Action | temple_owner (`admin`) | temple_user: finance | temple_user: staff | platform_owner: super_admin | platform_owner: support |
|---|---|---|---|---|---|
| ดู dashboard การเงิน | ✓ | ✓ | R(จำกัด) | — | — |
| donor CRUD | ✓ | ✓ | ✓ | — | — |
