...
describe("hall booking (จองศาลา) + monk invitations (นิมนต์พระ)", () => {
  ...
  it("clearing invited monks to [] while cancelled empties the join table + monkCount (re-activate is free)", async () => {
    const monk = await insertMonk(templeA, `พระล้างชุด-${randomUUID().slice(0, 6)}`);
    const date = "2031-06-09";
    const { ceremony: first } = await ceremonies.create(actorA, templeA, ip, {
      ceremonyType: "merit", title: "งานล้างพระ", ceremonyDate: date, monkPersonnelIds: [monk],
    });
    await ceremonies.update(actorA, templeA, ip, first.id, { status: "cancelled" });
    const { ceremony: cleared } = await ceremonies.update(actorA, templeA, ip, first.id, { monkPersonnelIds: [] });
    expect(cleared.monkCount).toBe(0);
    expect((await ceremonies.get(templeA, first.id)).invitedMonks).toEqual([]);
    // no monks left → re-activating clashes with nobody
    const { ceremony: planned } = await ceremonies.update(actorA, templeA, ip, first.id, { status: "planned" });
